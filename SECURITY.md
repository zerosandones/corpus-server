...
- The server reads `PORT` from the environment for HTTP. Optional HTTPS listener settings can be supplied through `corpus.config.yaml` (`https.enable`, `https.port`, `https.key-file`, `https.cert-file`).
