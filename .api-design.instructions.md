...
- **Path Sanitization & Security**:
  - All url slugs used for document paths must be sanitized and validated to prevent directory traversal and Local File Inclusion (LFI) vulnerabilities. Block any traversal paths containing `..` or targeting folders outside the storage folder with a `400 Bad Request` or `403 Forbidden` response
...
graph LR
    ...
    Request -->|Accept: application/json| JSON[Parsed JSON Metadata + Body]
```
...
- **`Accept: application/json`**:
  - Parses frontmatter on-the-fly or fetches from index cache.
  - Separates `frontmatter` properties and the markdown `body` into clean JSON keys.
  - Automatically lists child directories and files when a path represents a folder.
...
- **Comprehensive Indexes & Pagination**: 
  ...
  - To support crawling scaling, directories must implement standard limit-offset parameters (`?limit=100&offset=0`). The default page size MUST be exactly 100 items. The maximum allowed value for `limit` is 500; requests exceeding this must be clamped to 500.
  - Returned payloads must include metadata properties (`totalFiles`, `limit`, `offset`) to facilitate straightforward pagination logic for AI agents.
- **High-Performance Search**: The `/_search` endpoint must accept a simple string `q` and return a ranked list of matches carrying clean score and context snippet properties.
- **Semantic Consistency**: Keep schemas rigid. Once a frontmatter standard is defined (such as `title`, `description`, `tags`), keep it consistent. Do not mix camelCase and snake_case in API keys (always use standard camelCase or standard snake_case consistently).
